    ...
    function snapAppsFromDOMForTab(tabId){
      try {
        ...
        if(tabEl){
          ...
          var candidates = panel ? panel.querySelectorAll('[data-appid], [data-app-id]') : doc.querySelectorAll('[data-appid], [data-app-id]');
          candidates = Array.from(candidates).slice(0,2000);
          for (var j=0;j<candidates.length;j++){
            var a = candidates[j];
            var aid = a.getAttribute('data-appid') || a.getAttribute('data-app-id') || (a.dataset && (a.dataset.appid || a.dataset.appId)) || null;
