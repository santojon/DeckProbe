"""Scenario registry: every video scenario decorates a function with
`@register('name')` and `ALL_SCENARIOS` collects them. The runner explicitly
imports every scenario module so the decorators run. Mirrors
deckprobe.screenshots.lib.registry — same shape, so a scenario author
already familiar with screenshot scenarios needs nothing new here."""
