...
(async () => {
  ...
  const r = await send('Runtime.evaluate', { expression: 'JSON.stringify({ bpDiag: window.__ds_bp_diag, installed: window.__ds_input_installed, bpErr: window.__ds_input_bp_err })', returnByValue: true });
