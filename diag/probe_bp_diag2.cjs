...
(async () => {
  ...
  const r = await send('Runtime.evaluate', { expression: 'JSON.stringify({ diag: window.__ds_bp_diag, installed: window.__ds_input_installed, api: window.__ds_input_api, err: window.__ds_input_err, sources: window.__ds_input_sources, kbErr: window.__ds_bp_keydown_err })', returnByValue: true });
